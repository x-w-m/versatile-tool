    ...
    for i_li in range(len(li_elements)):
        ...
        if progress_pattern.search(link_text):
            ...
            for i, question in enumerate(questions, 0):
                ...
                # 获取题目ID
                question_id = question.get_attribute("qid")

                # 找到对应的选项容器
                options_container = form.find_element(By.XPATH,
                                                      f"//span[@qid='{question_id}']/following-sibling::div[@class='neiinput'][1]")
                ...
                # 判断题目类型
                question_type = "单选题" if options[0].get_attribute("type") == "radio" else "多选题"
                ...
                j = j + 1
            # 提交
            driver.find_element(By.ID, "btn_code").click()
            time.sleep(5)
        else:
            # 如果不匹配进度标识，则跳过
