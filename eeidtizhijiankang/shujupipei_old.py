...
# 2023年体测成绩表中无家庭住址选项
cj_column_names = ['年级编号', '班级编号', '班级名称', '学籍号', '民族代码', '姓名', '性别', '出生日期', '家庭住址',
                   '身高', '体重', '体重评分', '体重等级', '肺活量',
...
df_cj = pd.read_excel('体测成绩/2021.xls', names=cj_column_names)
...
cols_to_keep = ['学籍号', '姓名', '性别', '总分等级'] + list(df_cj.columns[10:41])
...
for col in df_mb.columns:
    if col.endswith('_cj'):
        ...
        # combine_first()方法可以填充缺失值，不为空的值不会被覆盖。
...
df_mb.to_excel('2021级2021年体测数据.xlsx', sheet_name='合并数据', index=False)
